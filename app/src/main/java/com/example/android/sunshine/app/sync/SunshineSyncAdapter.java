...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter
{
    ...
    private static final String OPEN_WEATHER_MAP_API_KEY = "afa43e3ef11e2f2bd0646916222a40db";
            ...
            mGoogleApiClient = new GoogleApiClient.Builder(context)
                    .addConnectionCallbacks(new GoogleApiClient.ConnectionCallbacks()
                    {
                        ...
                        public void onConnectionSuspended(int cause)
                            {
                                Log.e(LOG_TAG, "onConnectionSuspended: " + cause);
                            }
                    })
            ...
            try
                {
                    ...
                    Vector<ContentValues> cVVector = new Vector<ContentValues>(weatherArray
                            .length());

                    // OWM returns daily forecasts based upon the local time of the city that is
                    // being
                    // asked for, which means that we need to know the GMT offset to translate
                    // this data
                    // properly.
                    ...
                    int julianStartDay = Time.getJulianDay(System.currentTimeMillis(), dayTime
                            .gmtoff);
                    ...
                    for (int i = 0; i < weatherArray.length(); i++)
                        {
                            ...
                            JSONObject temperatureObject = dayForecast.getJSONObject
                                    (OWM_TEMPERATURE);
                            ...
                            if (i == 0)
                                {
                                    Log.e(LOG_TAG, "sending result");
                                    PutDataMapRequest putDataMapReq = PutDataMapRequest.create
                                            (PATH);
                                    putDataMapReq.getDataMap().putLong(REFRESHED_TIME, dateTime);
                                    putDataMapReq.getDataMap().putString(MAX_TEMP, Utility
                                            .formatTemperature(mcContext, high));
                                    putDataMapReq.getDataMap().putLong("sad", System
                                            .currentTimeMillis());
                                    putDataMapReq.getDataMap().putString(MIN_TEMP, Utility
                                            .formatTemperature(mcContext, low));
                                    Log.e("weaterid","weather id "+weatherId);

                                    Bitmap bitmap = BitmapFactory.decodeResource(mcContext
                                            .getResources(), Utility
                                            .getArtResourceForWeatherCondition(weatherId));
                                    ByteArrayOutputStream stream = new ByteArrayOutputStream();
                                    bitmap.compress(Bitmap.CompressFormat.PNG, 100, stream);
                                    byte[] byteArray = stream.toByteArray();
                                    putDataMapReq.getDataMap().putAsset(ASSET, Asset
                                            .createFromBytes(byteArray));
                                    PutDataRequest putDataReq = putDataMapReq.asPutDataRequest();

                                    PendingResult<DataApi.DataItemResult> pendingResult =
                                            Wearable.DataApi.putDataItem(mGoogleApiClient,
                                                    putDataReq);
                                    pendingResult.setResultCallback(new ResultCallback<DataApi
                                            .DataItemResult>()
                                    {
                                        @Override
                                        public void onResult(final DataApi.DataItemResult result)
                                            {
                                                Log.e(LOG_TAG, "Data onresult");
                                                if (result.getStatus().isSuccess())
                                                    {
                                                        Log.e(LOG_TAG, "Data item set: " + result
                                                                .getDataItem()
                                                                .getUri());
                                                    }
                                            }
                    ...
                    if (cVVector.size() > 0)
                        {
                            ...
                            getContext().getContentResolver().bulkInsert(WeatherContract
                                    .WeatherEntry.CONTENT_URI, cvArray);

                            // delete old data so we don't build up an endless history
                            getContext().getContentResolver().delete(WeatherContract.WeatherEntry
                                            .CONTENT_URI,
                                    WeatherContract.WeatherEntry.COLUMN_DATE + " <= ?",
                                    new String[]{Long.toString(dayTime.setJulianDay
                                            (julianStartDay - 1))});
    ...
    private void updateWidgets()
        {
            ...
            // Setting the package ensures that only components in our app will receive the
            // broadcast
    ...
    private void updateMuzei()
        {
            // Muzei is only compatible with Jelly Bean MR1+ devices, so there's no need to
            // update the
    ...
    private void notifyWeather()
        {
            ...
            String displayNotificationsKey = context.getString(R.string
            ...
            if (displayNotifications)
                {
                    ...
                    if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS)
                        {
                            // Last sync was more than 1 day ago, let's send a notification with
                            // the weather.
                            String locationQuery = Utility.getPreferredLocation(context);

                            Uri weatherUri = WeatherContract.WeatherEntry
                                    .buildWeatherLocationWithDate(locationQuery, System
                                            .currentTimeMillis());
                            ...
                            if (cursor.moveToFirst())
                                {
                                    ...
                                    int iconId = Utility.getIconResourceForWeatherCondition
                                            (weatherId);
                                    Resources resources = context.getResources();
                                    int artResourceId = Utility.getArtResourceForWeatherCondition
                                            (weatherId);
                                    String artUrl = Utility.getArtUrlForWeatherCondition(context,
                                            weatherId);

                                    // On Honeycomb and higher devices, we can retrieve the size
                                    // of the large icon
                                    // Prior to that, we use a fixed size
                                    @SuppressLint("InlinedApi")
                                    int largeIconWidth = Build.VERSION.SDK_INT >= Build
                                            .VERSION_CODES.HONEYCOMB
                                            ? resources.getDimensionPixelSize(android.R.dimen
                                            .notification_large_icon_width)
                                            : resources.getDimensionPixelSize(R.dimen
                                            .notification_large_icon_default);
                                    @SuppressLint("InlinedApi")
                                    int largeIconHeight = Build.VERSION.SDK_INT >= Build
                                            .VERSION_CODES.HONEYCOMB
                                            ? resources.getDimensionPixelSize(android.R.dimen
                                    ...
                                    NotificationCompat.Builder mBuilder =
                                            new NotificationCompat.Builder(getContext())
                                                    .setColor(resources.getColor(R.color
                                                            .primary_light))
                                                    .setSmallIcon(iconId)
                                    ...
                                    mNotificationManager.notify(WEATHER_NOTIFICATION_ID, mBuilder
                                            .build());
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon)
        {
            ...
            if (locationCursor.moveToFirst())
                {
                    int locationIdIndex = locationCursor.getColumnIndex(WeatherContract
                            .LocationEntry._ID);
                ...
                } else
                {
                    ...
                    // The resulting URI contains the ID for the row.  Extract the locationId
                    // from the Uri.
